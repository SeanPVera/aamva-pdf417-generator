...
import { ChevronDown, ChevronRight, BookOpen, Search, X, Check } from "lucide-react";
...
export const VersionBrowser: React.FC = () => {
  ...
  const filteredFields = useMemo(() => {
    ...
    if (!query) return versionDef.fields;
    return versionDef.fields.filter(
      (f) => f.code.toLowerCase().includes(query) || f.label.toLowerCase().includes(query)
    );
  }, [versionDef, searchQuery]);
        ...
        <div id="version-browser-panel" className="p-3 bg-white dark:bg-dark-surface space-y-3">
          ...
          <div className="space-y-2">
            ...
            {browsedVersion !== activeVersion ? (
              <button
                ...
                className="w-full text-xs font-semibold py-1.5 px-3 bg-brand-600 hover:bg-brand-700 text-white rounded transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
              >
                Set v{browsedVersion} as active
              </button>
            ) : (
              <div className="text-xs text-green-600 dark:text-green-400 font-medium flex items-center justify-between px-1 py-0.5">
                <span>Active form version</span>
                <Check size={14} />
              </div>
            )}
          </div>
          ...
          {versionDef && (
            <>
              ...
              {/* Search fields input */}
              <div>
                <label htmlFor="vb-search-input" className="sr-only">
                  Filter version fields
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-7 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 placeholder-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
                  />
                  {searchQuery && (
                    <button
                      type="button"
                      onClick={() => setSearchQuery("")}
                      className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-brand-500 rounded"
              ...
              <div className="overflow-auto max-h-64 rounded border border-gray-200 dark:border-dark-border">
                {filteredFields.length === 0 ? (
                  ...
                  >
                    No fields matching &lsquo;{searchQuery}&rsquo;
                  </div>
                ) : (
                  <table

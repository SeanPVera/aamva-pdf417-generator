...
/**
 * Length cap for a field.
 *
 * `field.maxLength` wins so a version can tighten a shared table entry: CDS
 * 2025 narrows DAK to nine while the table stays at eleven, which is what
 * keeps a dashed ZIP+4 valid everywhere else. Jurisdiction-defined subfile
 * elements still supply their own, because their codes only mean anything
 * inside one jurisdiction and putting them in the shared table would imply the

...
export function generateStateCardRevisionDate(stateCode?: string, issueDateStr?: string): string | null {
...
export function generateAAMVAPayload(
  ...
  const missing = mandatoryFields.filter(f => !dataObj[f.code]).map(f => `${f.label} (${f.code})`);
  ...
  if (dataObj.DAJ && dataObj.DAJ !== stateCode) {
    if (strictMode) throw new Error(`Strict Mode: DAJ (${dataObj.DAJ}) must match state code (${stateCode})`);
  ...
  if (length > 9999) throw new Error('Generated subfile exceeds 4-digit directory length limit (9999 bytes).');

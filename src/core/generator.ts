...
export function generateAAMVAPayload(
  ...
  // Work on an internal copy so we never mutate the caller's object.
  const data: Record<string, string> = { ...dataObj };
  ...
  if (AAMVA_STATE_RULES[stateCode]?.generators) {
    ...
    for (const [code, generator] of Object.entries(generators)) {
      if (options.autoGenerateDiscriminator && !data[code]) {
        data[code] = code === "DDB" ? generator(data.DBD) : generator();
        // Write generated values back so the caller can inspect them.
        dataObj[code] = data[code];
      }
    }
  }

  if (options.autoGenerateDiscriminator && !data.DCF && fields.some((f) => f.code === "DCF")) {
    data.DCF = generateDocumentDiscriminator();
    dataObj.DCF = data.DCF;
  }

  const mandatoryFields = getMandatoryFields(stateCode, version);
  const missing = mandatoryFields.filter((f) => !data[f.code]).map((f) => `${f.label} (${f.code})`);
  ...
  if (data.DAJ && data.DAJ !== stateCode) {
    if (strictMode)
      throw new Error(`Strict Mode: DAJ (${data.DAJ}) must match state code (${stateCode})`);
    data.DAJ = stateCode;
  }

  for (const field of fields) {
    if (data[field.code]) {
      let val = data[field.code];
  ...
  const crossFieldIssues = validateCrossFieldConsistency(data, fields);

...
- [ ] A reproducible byte layout for the decoded New York card. Its header declares a 323-byte DL subfile where the elements visible in the decode account for 224, so ~99 bytes of that subfile could not be accounted for from the material available (most likely fixed-width padding the field dump trimmed). NY's other findings — jurisdiction version 04, the `ZN` subfile, `DDD`, the alphanumeric `DCF`, the feet-inches height — are recorded; the byte layout is not, and NY therefore has no `issued` conformance vector.
